...
1. 实参的数目、数据类型、次序必须喝锁掉用的方法生命的形参列表匹配
...
2. 当Eden满了（达到一定比例）不能创建新对象，则会出发垃圾回收（GC），将无用对象清理掉，然后剩余对象复制到某个Survivor中，如S1,同时清空Eden区
...
1. 有抽象方法的一定是抽象类，但是抽象类里面可以定义普通方法，有构造器
